...
export default function AssessmentTypes() {
  ...
  const fetchTypes = async () => {
    setLoading(true);
    try {
      const res = await fetch('/api/admin/assessment-types');
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      if (editingType) {
        await fetch(`/api/admin/assessment-types/${editingType.id}`, {
          method: 'PATCH',
          headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify(formData),
        });
      } else {
        await fetch('/api/admin/assessment-types', {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify(formData),
        });
  ...
  const handleDeactivate = async (id: number) => {
    if (confirm('Tem certeza que deseja desativar este tipo?')) {
      try {
        await fetch(`/api/admin/assessment-types/${id}/deactivate`, { method: 'POST' });
        fetchTypes();
      } catch (error) {
  ...
  return (
    <div className="p-8 max-w-6xl mx-auto">
      <div className="flex justify-between items-center mb-6">
        <h1 className="text-2xl font-bold text-slate-900">Tipos de Avaliação</h1>
        <button
          onClick={() => openModal()}
      ...
      {isModalOpen && (
        <div className="fixed inset-0 bg-black/50 flex items-center justify-center p-4 z-50">
          <div className="bg-white rounded-lg shadow-xl max-w-md w-full p-6">
            <h2 className="text-lg font-bold mb-4">{editingType ? 'Editar Tipo' : 'Novo Tipo'}</h2>
